...
def dashboard(request):
    if request.method == 'POST':
        ...
        # Extract the time series
        # time_series = data["Time Series (5min)"][next(iter(dict))]
        # print(time_series)

        # difference = converted_open_stock_value - converted_amount
        # if difference >= 0:
        #     output_data = f"You gained £{abs(difference)}"
        # else:
        #     output_data = f"You lost £{abs(difference)}"
        output_data = "Hi"
        return render(request, 'dashboard.html', {'output_data': output_data, 'date': date, 'open': open_stock_value, 'amount': amount})
